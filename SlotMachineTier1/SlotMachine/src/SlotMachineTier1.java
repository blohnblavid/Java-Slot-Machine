...
public class SlotMachineTier1 {
    static class Symbol {
        ...
        final int weight, payout3;
        Symbol(String name, String display, int weight, int payout3) {
            this.name = name; this.display = display; this.weight = weight; this.payout3 = payout3;
        }
    ...
    static class Reel {
        ...
        Reel(List<Symbol> symbols, Random rng) {
            this.rng = rng;
            for (Symbol s : symbols) for (int i = 0; i < s.weight; i++) bucket.add(s);
        }
        Symbol spin() { return bucket.get(rng.nextInt(bucket.size())); }
    }

    static class Bank {
        int credits;
        Bank(int start) { credits = start; }
        boolean canBet(int bet) { return bet > 0 && credits >= bet; }
        void applyBet(int bet) { credits -= bet; }
        void applyWin(int win) { credits += win; }
        void reset(int start) { credits = start; }
    }

    static class SlotMachine {
        private final Reel[] reels;
        SlotMachine(Reel... reels) { this.reels = reels; }
    ...
    public static void main(String[] args) {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            while (true) {
                ...
                if (bet == 0) break;
                if (bet < 1 || bet > 50) { System.out.println("Bad bet. Try 1-50."); continue; }
                if (!bank.canBet(bet)) { System.out.println("Insufficient credits."); continue; }
...
// To compile and run: javac src/App.java && java -cp src App
// (Assuming you have Java installed and set up in your PATH)      